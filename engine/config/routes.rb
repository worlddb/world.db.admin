...
WorldDbAdmin::Engine.routes.draw do
  ...
  get '/:country_key/:key', :to => 'regions#shortcut', :as => :short_region_worker, :country_key => /[a-z]{2}/, :key => /[a-z]{2}/

  get '/tag/:key', :to => 'tags#shortcut', :as => :short_tag_worker, :key => /[a-z][a-z0-9_]*/
  ...
  get '/:key', :to => 'cities#shortcut', :as => :short_city_worker, :key => /[a-z]{3,}/
